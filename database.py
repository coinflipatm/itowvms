    ...
    try:
        with conn: # Use connection as a context manager for commit/rollback
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS vehicles (
                ...
                driver TEXT,
                reason_for_tow TEXT,
                status TEXT DEFAULT 'New', -- e.g., New, TOP Generated, Released, Auctioned
                owner_name TEXT,
                owner_address TEXT,
                owner_phone TEXT,
                owner_email TEXT,
            ...
            # TR-52/TR-208 columns removed as part of simplification to two-form system (TOP + Release Notice)
            ...
            # Add form generation timestamp columns if missing
            if 'top_form_generated_at' not in existing_cols:
                cursor.execute("ALTER TABLE vehicles ADD COLUMN top_form_generated_at TEXT")
                logger.info("Added column 'top_form_generated_at' to 'vehicles' table.")
            
            # Add form generated by columns if missing
            if 'top_form_generated_by' not in existing_cols:
                cursor.execute("ALTER TABLE vehicles ADD COLUMN top_form_generated_by TEXT")
                logger.info("Added column 'top_form_generated_by' to 'vehicles' table.")
            ...
            CREATE TABLE IF NOT EXISTS documents (
                ...
                document_type TEXT, -- e.g., 'TOP', 'Release Notice', 'Photo', 'Owner Letter'
            ...
            # Police Log table for form submissions
            conn.execute("""
            CREATE TABLE IF NOT EXISTS police_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                vehicle_id INTEGER NOT NULL,
                towbook_call_number TEXT,
                form_type TEXT NOT NULL, -- e.g., TOP, Release Notice
...
def update_vehicle_status(call_number, new_status, update_fields=None):
    from utils import log_action, calculate_next_auction_date, calculate_newspaper_ad_date
...
def insert_vehicle(data):
    ...
    try:
        ...
        columns = [
            ...
            'certified_mail_number_lienholder', 'newspaper_ad_date', 'jurisdiction'
        ]
            ...
            try:
                ...
                if tow_date_str and tow_date_str != 'N/A':
                    ...
                    if tow_date:
                        ...
                        # Handle TOP Generated status - after 20 days, can move to auction/scrap
                        if vehicle['status'] == 'TOP Generated' and vehicle['top_form_sent_date']:
                            ...
                            if top_date:
                                ...
                                # Calculate days until eligible for auction/scrap (20 days)
                                if days_since_top < 20:
                                    ...
                                    # Vehicle is now eligible for auction or scrap decision
                                    cursor.execute("""
